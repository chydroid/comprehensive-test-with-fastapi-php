...
| A5 | **题库批量导入（Excel/Word）** | 标配 | 仅学生可批量导入；题目只能单条录入 | 题库建设成本极高，教师录题痛苦 | **✅ 已完成（2026-09-19，即交付清单里的 B3）** |
...
| B4 | **成绩公示与隐私分级** | 常见 | 成绩可查但缺「按班级/个人粒度控制可见性」 | 兼顾公开性与隐私的精细控制不足 | **✅ 已完成（2026-09-19）** |
...
### ✅ B4 成绩公示与隐私分级（2026-09-19 完成）

**设计要点：可见性判定只在一处**

成绩公示解决的是「同场考生之间能不能互相看到」，不是「任何人能查任意考试的分数」。
判定全部收敛在只读服务 `ScoreBoard` 里，前端只负责渲染它给出的 `rows` / `me`：

| 硬规则 | 理由 |
|---|---|
| 只统计**已交卷**考生（`stu_status LIKE 'over%'`） | 未交卷者没有最终成绩，进榜会把「还在答题的人」显示成 0 分（与 `ScoreAnalysis` 同口径） |
| 查看者**必须是本场考生**（有 `stuscore` 行） | 否则任何考生枚举 `exam_id` 就能刷出全校各场考试的分数 —— 那不是公示，是数据泄露 |
| 只有**已结束**的场次才公示 | 进行中的分数还在变，张榜即误导 |
| 名次为**全场名次**（竞赛排名，并列同名次） | 班级粒度只是在其上做展示过滤；若按班内重排名，同一分数会在本班榜与全场榜显示两个名次 |

粒度由 `examinfo.score_visibility` **逐场**配置，默认 `private`（＝既有行为，零行为变化）：
`private` 仅本人 / `class` 本班同学可见 / `public` 全体考生可见。非法值一律回落 `private`
（`Exam::visibilityOf()`）—— 配置写错时的失败方向必须是「更保守」。

| 层 | 改动 |
|---|---|
| 数据 | `examinfo.score_visibility varchar(16) NOT NULL DEFAULT 'private'` |
| 服务 | `app/Services/ScoreBoard.php`（`forExam()` 返回 `allowed/reason/visibility/scope_label/can_view_others/pass_score/me/rows`） |
| 模型 | `Exam::VIS_PRIVATE/VIS_CLASS/VIS_PUBLIC` + `SCORE_VISIBILITY_LABELS` + `visibilityOf()` + `passPercent()`/`passScoreOf()`/`isPassed()` |
| 接口 | `GET /api/student/score-board?exam_id=N`；`GET /api/student/scores` 每行附 `score_visibility`/`retake_of`/`cert_threshold`/`cert_no`/`can_view_board`（已 join，不额外查库） |
| 前端 | 双端考试编辑器新增「成绩公示与电子证书」卡片（分段控件 + 说明文案）；考生「我的成绩」新增「成绩榜」按钮 → 榜单弹窗（我的成绩卡 + 排名行，`is_me` 高亮） |
| 设置 | 新增运行参数 `exam_pass_percent`（默认 60，范围 1–100，`public`）作为全站默认及格线 |

**验证**：`publish_cert_retake_test.php` A1–A7（private/class/public/非法回落/非本场拒绝/未结束不公示/列表携带字段）。

---

### ✅ C1 电子证书（2026-09-19 完成）

| 决策 | 说明 |
|---|---|
| **惰性签发，不依赖定时任务** | 本项目无常驻 cron。「考试结束 → 判分 → 发证」的前两步已由惰性流转完成，发证同样挂在读路径：考生打开「我的证书」时把该考生所有**已结束且已达标、尚未签发**的场次补签一遍，再返回全部证书。不存在「考完了却没证」的时序窗口 |
| **幂等靠唯一键，不靠「先查后插」的逻辑正确性** | `certificate` 上有 `UNIQUE(exam_id, stu_id)`，并发双击页面在数据库层被拒绝；捕获唯一键冲突后回查即可，无需事务或行锁 |
| **正文是快照** | 考试名称、科目、得分、达标分、满分为签发时写入 `certificate` 行，不随源数据变动 —— 考试改名、题库清理、成绩归档都不该让一张已发出的证书变样。这正是「证书」与「成绩查询」的本质区别：前者是凭证，后者是视图 |
| **编号不可枚举** | `CT` + 8 位日期 + 32 位随机十六进制（42 字符）。刻意不做「考试号+准考证号」这类可推导编号 —— 否则核验接口等于把「写脚本遍历准考证号即可确认某人是否通过某场考试」变成可能 |
| **公开核验只凭编号 + 姓名脱敏** | 随机段让「知道编号」等价于「持有证书」，无需验证码；姓名按 `ScoreController::maskName` 同规则脱敏（首尾保留、中间打星），既能核实「这张证是不是我的」，又不把全名公开在互联网上 |
| **达标分用绝对分** | 发证是考务方按场次性质的自主决定，逐场填写最直观；`cert_threshold = 0` 表示本场不发放。配置校验：达标分不得高于满分（错误码 40006） |

| 层 | 改动 |
|---|---|
| 数据 | 新表 `certificate`（快照列 + `UNIQUE(exam_id,stu_id)` + `UNIQUE(cert_no)`）；注意 `cert_no` 必须是 `varchar(64)` —— 首版写成 `varchar(32)` 会静默截断导致插入报 1406、证书永远签不出来 |
| 服务 | `app/Services/Certificate.php`：`issue()`（惰性幂等）/ `forStudent()`（补签后返回）/ `findOne()`/`findByNo()`/`verify()`（脱敏）/ `maskName()`/`newCertNo()`/`thresholdOf()`/`isEnabled()`/`qualifies()` |
| 模型 | `app/Models/Certificate.php`；`examinfo.cert_threshold` |
| 接口 | `GET /api/student/certificates`（我的证书 + 统计）/ `GET /api/student/certificates/{examId}`（单张，不可签发一律 **404** 避免探测）/ `GET /api/public/certificates/verify?cert_no=`（免登录，`valid=false` 表达查不到） |
| 前端 | 考生中心新增「我的证书」页（统计卡 + 证书卡列表 + 查看弹窗 + 复制编号 + 打印/另存 PDF）；门户新增公开「证书核验」页与导航入口 |

**验证**：`publish_cert_retake_test.php` B1–B8（未启用为空 / 达标惰性签发幂等 / 单张 / 公开核验脱敏 / 未知编号 / 快照不随改名而变 / 编号不可枚举 / 达标分高于满分被拒）；`publish_cert_retake_frontend_test.php` 七（公开核验免登录 200、其余新接口未登录 401）。

---

### ✅ C2 补考 / 重考（2026-09-19 完成）

**核心决策：补考不是给原考试加一个状态，而是另开一场正式考试。**

用 `examinfo.retake_of` 指向源场次、用 `exam_retake_stu` 限定参考名单。收益：

- 原场次的成绩、名单、监考记录一概不动，**原始考核记录可追溯、可审计**；
- 补考直接复用「开放入场 → 出题 → 开考 → 判分 → 结束 → 发证」整条既有生命周期，不需要为它另写状态机；
- 补考成绩天然出现在成绩单、成绩分析、监考模块里，无需各处特判。

代价是多一张名单表 + 两处名单分支。这两处刻意**集中**而不是散落：

| 分叉点 | 文件 |
|---|---|
| 出题给谁 | `Exam::studentIdsForExam()`（补考走名单，普通场次走班级） |
| 谁能入场 | `Exam::isStudentEligible()`（补考只认名单；不提供准考证号直接拒绝） |
| 待考列表 | `Exam::pendingForStudent()`（「班级匹配 OR 补考名单命中」的 EXISTS 子查询） |
| 组卷入口 | `ExamEngine::generateForClass()` 改用 `studentIdsForExam()` |

**名单口径**：「未通过」= 已交卷但得分低于及格分；「缺考」= 无成绩行或未交卷；「已通过」= 达线。
及格线取**得分率百分比**（`Exam::passPercent()`，默认 60、可配、请求可覆盖）—— 各场满分差异很大，
50 分卷与 200 分卷的「60 分」含义完全不同。

**保护性校验**（缺一即 400）：

- 源考试必须**已结束**才谈得上「谁没通过」；
- 补考开始/结束时间必填、且结束必须晚于开始；
- 名单必须落在源场次的参考范围内（防止把无关考生塞进补考）；
- 空名单拒绝；
- **已通过者默认拒绝**进入补考，需显式 `allow_passed=1`（补考是给未通过者的第二次机会）。

| 层 | 改动 |
|---|---|
| 数据 | `examinfo.retake_of int unsigned DEFAULT NULL`；新表 `exam_retake_stu`（`UNIQUE(exam_id,stu_id)`） |
| 服务 | `app/Services/ExamRetake.php`：`candidates()`（状态判定 + 未通过/缺考排前）/ `create()`（事务内建考试 + 复制 manual/by_kp 明细 + 写名单）/ `sourceOf()` / `clampPercent()` |
| 接口 | `GET|POST /api/{admin,teacher}/exams/{id}/retake-candidates|retake`；新增写接口登记到 `SessionAuthMiddleware::WRITE_POINTS`（`exam.add`） |
| 权限 | 教师端 `{id}` 越权一律 **404**（防枚举探存在性）；教师端生成时**强制 `exam_tea = 本人`**，忽略前端传值 |
| 前端 | 共享弹窗 `views/retake.js`（候选名单统计卡 + 勾选 + 「仅选未通过+缺考/全选/清空」快捷操作 + 时间填写 + 勾到已通过者自动带 `allow_passed`）；管理端与教师端列表行内新增「补考」按钮（仅已结束场次显示）与「补考自 #N」标签 |
| 级联 | 删除考试时一并清理 `exam_retake_stu`；夹具 `cleanup()` 同步回收名单与证书 |

**验证**：`publish_cert_retake_test.php` C1–C12（候选状态 / 生成复制配置与名单 / 拒绝已通过·名单外·空·未结束·时间非法 / 准入只认名单 / 出题只给名单 / 考生端可见性 / 教师端归属与越权 404 / 删除级联 / 补考场次同样公示与发证 / CSRF 保护）。
...
| 项 | 内容 |
|---|---|
| 新增文件 | `app/Services/{ScoreBoard,Certificate,ExamRetake}.php`、`app/Models/Certificate.php`、`app/Controllers/{StudentCertificateController,PublicCertificateController}.php`、`public/assets/js/views/{retake,cert-verify}.js`、`public/assets/js/views/student/certificates.js`、`db/add_publish_cert_retake.sql`、`temp/apply_publish_cert_retake_ddl.php`、`test/cases/publish_cert_retake_test.php`、`test/cases/publish_cert_retake_frontend_test.php` |
| 全量回归 | **1414 PASS / 0 FAIL / 1 SKIP**（本轮新增 138 + 79 断言，零回归） |
